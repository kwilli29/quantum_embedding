...
and a Layout-Aware embedding flow using minorminer on faulty Pegasus.

A certain ratio of nodes is removed from the specified topology,
and the three algorithms are run 10 times on the constant faulty topology

Each ratio the topology resets and different nodes are removed for the next ratio
"""
...
ratio_broken_verts = [0.25,0.4,0.5]
...
for rat in num_broken_verts:
    ...
    broken_graph = generators.p16_graph() # only reset every new ratio, not every run
    ...
    for p in range(num_runs):    
        ...
        if p>7:
            ...
            plt.tight_layout()
            plt.savefig(f'../Desktop/BigCompleteLayoutAware/Pegasus/{rat}_{d}brknComp_layout_aware_agn_bp16')
            plt.clf()
            ...
            plt.tight_layout()
            plt.savefig(f'../Desktop/BigCompleteLayoutAware/Pegasus/{rat}_{d}brknComp_layout_aware_mig_bp16')
            plt.clf()
            ...
            plt.tight_layout()
            plt.savefig(f'../Desktop/BigCompleteLayoutAware/Pegasus/{rat}_{d}brknComp_layout_aware_nomig_bp16')
            plt.clf()
            ...
            plt.tight_layout()
            plt.savefig(f'../Desktop/BigCompleteLayoutAware/Pegasus/{rat}_{60}brknComp_layout_agn_hist_bp16')
            plt.clf()
            ...
            plt.tight_layout()
            plt.savefig(f'../Desktop/BigCompleteLayoutAware/Pegasus/{rat}_{d}brknComp_layout_awaremig_hist_bp16')
            plt.clf()
            ...
            plt.tight_layout()
            plt.savefig(f'../Desktop/BigCompleteLayoutAware/Pegasus/{rat}_{d}brknComp_layout_awarenomig_hist_bp16')
            plt.clf()
    ...
    with open(f'../Desktop/BigCompleteLayoutAware/Pegasus/{rat}_{d}brknComp_dataFaults_bp16.txt', 'w') as f:
