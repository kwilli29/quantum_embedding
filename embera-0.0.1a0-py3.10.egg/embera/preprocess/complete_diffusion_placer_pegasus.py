## KW ##
# Adapted diffusion_placer.py to be able to process 
# the 3 coordinate tile system of Pegasus
########
...
from embera.preprocess.tiling_parser_pegasus import DWaveNetworkXTiling
...
class DiffusionPlacer(DWaveNetworkXTiling):
    """ Diffusion-based migration of a graph layout for Pegasus
    """
    def __init__(self, S, Tg, **params):
        ...
        ## KW ##
        self.t_size = len(Tg) # t_size = Target size (source --> description)
        self.m = Tg.graph['rows'] # mxn = dimensions of tiled architecture
        self.n = Tg.graph['columns'] 
        self.shape = (3,self.m,self.n) # pegasus assumption
        ########
    ...
    def _assign_candidates(self):
        ...
        for s_node, s_tile in self.mapping.items():
            ...
            ## KW ##
            # added a .neighbors attribute to Tile 

            dim, i, j = s_tile
            
            # Neighbouring tiles
            top, bottom, top_left, top_right, bottom_right, bottom_left = self.tiles[s_tile].neighbors(dim, i, j, self.m, self.n) #neigh
            ########
            ...
            if self.vicinity == 1:
                ...
                for tile in [top, bottom]:
                    ## KW ##
                    candidates[s_node] = self.tiles[s_tile].qubits
                    # original: candidates[s_node].update(self.tiles[tile].qubits)
                    ########
            elif self.vicinity == 2:
                # Extended neighbors
                for tile in [top, bottom, top_left, top_right, bottom_right, bottom_left]:
                    ## KW ##
                    candidates[s_node] = self.tiles[s_tile].qubits
                    # original: candidates[s_node].update(self.tiles[tile].qubits)
                    ########
                ...
                d, x_coord, y_coord = self.layout[s_node]
                d, i_index, j_index = s_tile
                if x_coord >= j_index+0.5:
                    if y_coord >= i_index+0.5: neighbor_tiles = (bottom, bottom_right)
                    else: neighbor_tiles = (top,top_right)
                else:
                    if y_coord >= i_index+0.5: neighbor_tiles = (bottom, bottom_left)
                    else: neighbor_tiles = (top,top_left)

                for tile in neighbor_tiles:
                    ## KW ##

                    if tile in self.tiles: # added if
    ...
    def _scale(self):
        ...
        for d, sx, sy in self.layout.values():
        ...
        for s_node, s_coords in self.layout.items():
            ...
            self.layout[s_node] = (d, scaled_x, scaled_y)
            tile = self._coords_to_tile(d, scaled_x, scaled_y)
    ...
    def _coords_to_tile(self, d, x_coord, y_coord):
        """ Tile values are restricted.
        Horizontallly 0<=i<=n
        Vertically 0<=j<=m

        """
        # return a valid Pegasus coordinate based on given x and y

        j = max(min(math.floor(x_coord), self.n-1), 0)
        i = max(min(math.floor(y_coord), self.m-1), 0)
        k = max(min(math.floor(d), len(self.shape)-1), 0) #

        # corner cases
        if i == 15 and j == 15 and d==2:
            i = 15
            j = 14
        if i == 15 and j == 15 and (d==1 or d==0):
            i = 14
            j = 15
    ...
    def _get_attractors(self, d, i, j):
        ...
        # added a .neighbors attribute to Tile        
        # Neighbouring tiles
        top, bottom, top_left, top_right, bottom_right, bottom_left = self.tiles[(d,i,j)].neighbors(d, i, j, self.m, self.n) #neigh
        ########
        ...
        if lh:
            return (top, top_left) if lv else (bottom, bottom_left)
        # else
        return (top, top_right) if lv else (bottom, bottom_right)
    ...
    def _get_gradient(self, tile):
        ...
        if d_ij == 0.0 or tile.index == None:
            return 0.0, 0.0
        
        v, hv = self._get_attractors(*tile.index) # no horizontal neighbors in pegasus
        # h, v, hv = self._get_attractors(*tile.name)

        # added if for corner neighbors that don't have 8 neighbors
        # do not include: d_h = self.tiles[h].concentration if h in self.tiles else 0
        d_v = self.tiles[v].concentration if v in self.tiles else 0
        d_hv = self.tiles[hv].concentration if hv in self.tiles else 0
        ########
        gradient_x = - (d_lim - (0 + 0.5*d_hv)) / (2.0 * d_ij)
    ...
    def _map_tiles(self):
        ...
        for tile in self.tiles.values():
            if tile.supply:
                ...
                # original: tile.concentration = len(tile.nodes)/tile.supply
                ########
