...
class DiffusionPlacer(DWaveNetworkXTiling):
    ...
    def _assign_candidates(self):
        ...
        for s_node, s_tile in self.mapping.items():
            ...
            #neigh = DWaveNetworkXTiling.get_tile_neighbors(self, self.tiles[s_tile].qubits)
            # neigh = DWaveNetworkXTiling.get_tile_neighbors(self, self.tiles[s_tile].index)
            i, j = s_tile
            
            # Neighbouring tiles
            n, s, w, e, nw, ne, se, sw = self.tiles[s_tile].neighbors(i, j, self.m, self.n) #neigh
            ########
            ...
            if self.vicinity == 1:
                ...
                for tile in [n,s,w,e]:
                    candidates[s_node] = self.tiles[s_tile].qubits
                    # candidates.update(s_node=self.tiles[tile].qubits)
    ...
    def _get_attractors(self, i, j):
        ...
        #neigh = DWaveNetworkXTiling.get_tile_neighbors(self, self.tiles[(i,j)].qubits)
        #neigh = DWaveNetworkXTiling.get_tile_neighbors(self, self.tiles[(i,j)].index)
        
        # Neighbouring tiles
        n, s, w, e, nw, ne, se, sw = self.tiles[(i,j)].neighbors(i, j, self.m, self.n) #neigh
        ########
    ...
    def _get_gradient(self, tile):
        ...
        if d_ij == 0.0 or tile.index == None:
            return 0.0, 0.0
        h, v, hv = self._get_attractors(*tile.index)
        # h, v, hv = self._get_attractors(*tile.name)
        ########

        ## KW ## 
        # added if for corner neighbors that don't have 8 neighbors
        d_h = self.tiles[h].concentration if h in self.tiles else 0
    ...
    def _map_tiles(self):
        ...
        for s_node, s_coords in self.layout.items():
            ...
            ## KW ##
            # self.tiles[tile].qubits.remove(s_node)
            self.tiles[tile].nodes.remove(s_node)
            # self.tiles[new_tile].qubits.append(s_node)
